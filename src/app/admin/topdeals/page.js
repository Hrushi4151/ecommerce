...
const page = () => {
  ...
  const handleButton = async () => {
    ...
    console.log(ids)
    const res=fetch("/api/addTopDeals", {
      method: "POST",
      headers: {
  ...
  const handleDeleteButton = async () => {
    ...
    console.log(ids)
    const res=await fetch("/api/deleteTopDeals", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <>
      <div className="productspage h-[calc(100vh-2.5rem)] relative m-5 rounded-2xl flex flex-col justify-between items-center overflow-auto hide-scrollbar">
       ...
       {data.length>0 ? <TopDealsCarousel/> : <div className="text-2xl font-bold text-center mb-8">TOP DEALS</div>}
